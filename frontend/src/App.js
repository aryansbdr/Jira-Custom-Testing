    ...
    async function loadContext() {
      ...
      try {
        ...
        if (
          ...
          // Restore saved draft recommendations from localStorage cache if present
          try {
            const saved = localStorage.getItem(`subtask_recs_${key}`);
            if (saved) {
              ...
              if (Array.isArray(parsed) && parsed.length > 0) {
                ...
                if (remaining > 0) {
                  ...
                  console.log('[App] Restored draft recommendations from localStorage for', key);
                }
              }
            }
          } catch (e) {
            console.warn('[App] Failed to load localStorage cache:', e);
          }
    ...
    async () => {
      ...
      try {
        ...
        if (data && typeof data === 'object' && data.success === false) {
          console.warn('[App Diagnostik] Resolver & BE Terhubung 100%, namun AI error:', data);
          setGroups([]);
          setStatusType('error');
        ...
        try {
          ...
          console.warn('[App] Failed to save localStorage cache:', e);
        }
      ...
      {loading && (
        ...
        <div
          style={{
            ...
            bottom: 0,
            backgroundColor:
              'var(--ds-surface-overlay, rgba(23, 43, 77, 0.85))',
            display: 'flex',
                            ...
                            >
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', minWidth: 0 }}>
                                ...
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', alignItems: 'center', marginTop: '2px' }}>
                                  {(task.parent_summary || (task.parent_key && task.parent_key !== issueKey)) && (
                                    <span
                                      style={{
                                        fontSize: '11px',
                                    ...
                                    >
                                      Story: <strong style={{ color: 'var(--ds-text, #172B4D)', fontWeight: 600 }}>{task.parent_key ? `[${task.parent_key}] ` : ''}{task.parent_summary || task.parent_key}</strong>
                                    </span>
                                  )}
                                  {task.assigneeName && (
                                    <span
                                    ...
                                    >
                                      👤 {task.assigneeName} {task.assigneeRole ? `(${task.assigneeRole})` : ''}
                                    </span>
                                  )}
                                </div>
                              </div>
