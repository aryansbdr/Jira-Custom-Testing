...
function App() {
  ...
  useEffect(() => {
    view.getContext().then((context) => {
      if (context && context.extension && context.extension.issue) {
        ...
        // Matikan loading jika diakses di luar tiket Jira
        setLoading(false);
      }
    });
  }, []);
  ...
  return (
    <>
      ...
      {/* Bagian Diagram Squad Progress */}
      <div className="App" style={{ marginTop: '20px', borderTop: '1px solid #DFE1E6', paddingTop: '16px' }}>
        <SquadReport />
      </div>
    </>
  );
