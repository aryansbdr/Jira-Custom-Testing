...
class GeminiLlmClient(ILlmClient):
    ...
    def generate_subtasks_from_ac(
        ...
        prompt = (
            ...
            "2. BE Tasks: Use exact AC wording. If no BE details in AC, output exactly 1 subtask: 'BE - Design Spec API for <story_name>'. NEVER invent URL paths (/v1/...), class names, or migration scripts.\n"
            "3. Consolidation: Group by LOGICAL section/component/modal/page, NOT per field/bullet. Merge all business logic, calculations, and rules directly into their respective component/chart/page subtask. NEVER create separate subtasks for logic/calculations.\n"
            "4. Banned words: Ensure/Handle/Verify/Validate/Make sure/Check that/schema/Review code\n"
            "5. SP: Fibonacci only (0, 0.5, 1, 2, 3, 5, 8, 13)\n\n"
        )
        ...
        if examples:
            prompt += "EXAMPLES:\n"
            for eg in examples:
                subs = ", ".join(f"{s['summary']}({s['role'][0].upper()})" for s in eg["subtasks"])
        ...
        # Multi-Model Automatic Fallback Chain (prioritizing 2.5-flash for max speed & accuracy)
        fallback_models = ["gemini-2.5-flash", settings.GENERATION_MODEL, "gemini-2.0-flash", "gemini-2.5-flash-lite", "gemini-2.0-flash-lite"]
