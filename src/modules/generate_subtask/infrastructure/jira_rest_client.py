...
class JiraRestClient(IJiraClient):
    ...
    def create_subtask_issue(
        ...
        if response.status_code not in (200, 201):
            ...
            if response.status_code not in (200, 201):
                raise Exception(
                    f"Failed to create subtask in Jira ({response.status_code}): {response.text}"
                )
