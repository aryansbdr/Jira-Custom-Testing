...
class ExcelReporter:
    ...
    def generate_progress_report(
        ...
        sprint_info = report_data.get("sprint_info") 
        sprint_name = sprint_info.get("name") or "Active Sprint"
        days_rem = sprint_info.get("days_remaining")
        
        if days_rem is not None:
            end_date_str = (sprint_info.get("end_date"))[:10]
            sprint_sub = f"{root_summary} | Sisa Waktu Sprint: {days_rem} Hari Lagi (Due: {end_date_str}) | Diekspor pada: {now_str}"
        else:
        ...
        kpi_vals = [
            sprint_name,
            f"{days_rem} Hari Lagi" if days_rem is not None else "-",
            epics_count,
            overall.get("total", 0),
            overall.get("todo", 0),
        ...
        for idx, sub in enumerate(detailed_subtasks, 1):
            ...
            st_cat = sub.get("status_category", "To Do")
            st_name = sub.get("status", st_cat)
            c_st = ws.cell(row=d_row, column=8, value=st_name)
            c_st.alignment = Alignment(horizontal="center", vertical="center")
            if st_cat == "Done":
                c_st.fill = fill_status_done
                c_st.font = font_status_done
            elif st_cat == "In Progress":
